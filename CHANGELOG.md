...
### Changed
- Gray chrome with one cyan accent, replacing the violet wash: the background is still never painted (`Color::Reset`), structure comes from gray — hairlines `#3A3A3A`, filled charcoal panels `#141414`, dim `#6B7280` secondary copy, white primary copy — and cyan `#7DD3FC` appears only on the focused selection (the `>` caret and the selected label on the dark gray `#262626` bar). Green `#4ADE80` covers `✓` success and `+N` diff additions; red and amber stay on diagnostics; the Thinking status is a muted gold. `#A78BFA` and `#221A38` are banned everywhere
...
- Login, trust, `/mode`, `/permissions`, permission prompts, plan / clear / delete confirms and questions are numbered pickers: `> 1 …` cyan on the selected row, `· 2 …` white on the others, dim descriptions under the titles, `↑↓ select · ↵ confirm · esc …` hints; the sign-in screen reads `Welcome to Cortex CLI!` / `How would you like to log in?`
...
- All 50 lock states recaptured at 40×12 and 120×40 on the gray chrome (raw and macOS Terminal composites)
