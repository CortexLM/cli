...
and autonomy level. Turns run on the **Cloud** Code runtime by default, so a
fresh install can complete a turn without extra configuration. To run tools on
This PC or over SSH, set `CORTEX_COMPUTER` (see
[Environment variables](../configuration/env.md)). Those runtimes need an
already connected Code session; Cortex will not substitute Cloud. Type what you
want changed and press `Enter`.
