...
- Boards: [`index.md`](index.md) — **109** runtime boards at 120×40, 57 of them also at 40×12. Designer PNG files under `{40x12,120x40}/` from other PRs are not rewritten; new scenes are captured with `generate_tui_lock_screenshots --v2 --only`.
...
| COR-35 batch: headless CI · permission DSL · checkpoints · CI cookbook · JSON Schema · teleport · review-only · marketplace · sandbox allowlist · auto-approval · PR apply-back · ACP · stdin stream | `bare-ci`, `ci-cookbook`, `permission-rules`, `checkpoint-rewind`, `json-schema`, `cloud-teleport`, `review-only`, `plugin-marketplace`, `sandbox-allowlist`, `auto-approval`, `pr-apply-back`, `acp-editor`, `stdin-multiturn` |
