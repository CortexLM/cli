...
pub use mention::{
    AgentMention, BUILTIN_SUBAGENTS, MENTION_SYSTEM_PROMPT, ParsedAgentMessage,
    extract_mention_and_text, find_first_valid_mention, parse_agent_mentions,
    parse_message_for_agent, starts_with_mention,
};
...
pub use collab::{
    AgentGuard, CloseAgentArgs, CloseAgentResult, CollabError, CollabResult,
    DEFAULT_WAIT_TIMEOUT_MS, MAX_CONCURRENT_AGENTS, MAX_THREAD_SPAWN_DEPTH, MAX_WAIT_TIMEOUT_MS,
    MIN_WAIT_TIMEOUT_MS, PendingInput, SendInputArgs, SendInputResult, SpawnAgentArgs,
    SpawnAgentResult, SpawnConfig, WaitArgs, WaitResult,
};

// Re-export task types
pub use task::{
    DagBuilder, DagError, DagHydrator, DagResult, DagStore, ExecutionProgress, ExecutorConfig,
    ExecutorError, ExecutorResult, InMemoryDagStore, PersistenceError, SerializedDag,
    SessionHydrationError, SessionHydrationResult, SessionHydrator, SessionRestoreConfig,
    StaleTaskChecker, StaleTaskInfo, Task, TaskDag, TaskExecutionResult, TaskExecutor, TaskId,
    TaskSpec, TaskStatus, TaskStore, execute_dag,
};

// Re-export routing types
pub use routing::{
    DispatchMode, RoutingDecision, TaskInfo, can_parallelize, decide_routing, estimate_duration,
};
