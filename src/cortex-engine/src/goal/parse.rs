...
///
/// Reserved single tokens: `pause`, `resume`, `clear`, `status`. Anything else
...
pub fn parse_goal_args(args: &[String]) -> Result<GoalCommand, String> {
    ...
    if args.len() == 1 {
        match args[0].to_ascii_lowercase().as_str() {
            ...
            "status" => return Ok(GoalCommand::Status),
...
mod tests {
    ...
    fn empty_is_status() {
        ...
        assert_eq!(parse_goal_input("  /goal  ").unwrap(), GoalCommand::Status);
        assert_eq!(
            parse_goal_input("/goal status").unwrap(),
            GoalCommand::Status
        );
        assert_eq!(
            parse_goal_input("/GOAL STATUS").unwrap(),
    ...
    fn set_objective() {
        ...
        assert_eq!(
            parse_goal_input("/goal status the rollout").unwrap(),
            GoalCommand::Set {
                objective: "status the rollout".into()
            }
        );
    }
    ...
    fn rejects_non_goal() {
        ...
        assert!(parse_goal_input("/goalstatus").is_err());
    }
}
