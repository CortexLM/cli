...
#[test]
fn test_service_unavailable_is_product_facing() {
    let message = "The coding service is temporarily unavailable";
    assert_eq!(
        CortexError::BackendUnavailable(
            "reqwest: error sending request for url (https://api.openai.com)".into()
        )
        .user_friendly_message(),
        message
    );
    assert_eq!(CortexError::Timeout.user_friendly_message(), message);
    assert_eq!(
...
#[test]
fn test_auth_message_does_not_name_providers() {
    let msg = CortexError::ApiKeyNotFound {
        provider: "openai".to_string(),
    }
    .user_friendly_message();
