...
mod tests {
    ...
    fn snapshot_login_pending_shows_cortex_auth() {
        ...
        Modal::render(&modal, area, &mut buf);
        let text = buffer_text(&buf);
        assert!(
            text.contains("Login to Cortex"),
            "missing title in snapshot:\n{text}"
        );
        assert!(
    ...
    fn snapshot_login_failed_is_product_facing() {
        ...
        modal.set_failed("The coding service is temporarily unavailable".to_string());
        let area = Rect::new(0, 0, 72, 16);
        let mut buf = ratatui::buffer::Buffer::empty(area);
        Modal::render(&modal, area, &mut buf);
        let text = buffer_text(&buf);
        assert!(
            ...
            "missing product-facing error in snapshot:\n{text}"
        );
        assert!(!text.to_lowercase().contains("reqwest"));
        assert!(!text.to_lowercase().contains("openai"));
        assert!(!text.to_lowercase().contains("grok"));
    }
}
